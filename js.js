...
`
        ...
        cc.addEventListener("click", (eo) => {
            if (eo.target.classList.contains("btnsmall")) {
                eo.target.parentElement.remove()
                total = 0

                const pshirt = eo.target.parentElement.querySelector(".pshirt").innerText
                const all = document.querySelectorAll(".divconten")
                all.forEach(item => {
                    const ite = item.getElementsByClassName("shirt")[0].innerText
                    console.log(ite)
                    if (pshirt == ite) {
                        const bttt = item.getElementsByClassName("bt")[0];
                        bttt.classList.remove("google");
                        bttt.innerHTML = ("click")
                    }
